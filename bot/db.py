...
class DB:
    ...
    def add_to_queue(self, tg_user_id: int, vacancy_id: int, score: int) -> None:
        with self.conn() as c:
            c.execute(
                "INSERT OR IGNORE INTO user_queue(tg_user_id, vacancy_id, score, status) VALUES(?,?,?,'new')",
    ...
    def next_queue_item(self, tg_user_id: int) -> Vacancy | None:
        with self.conn() as c:
            row = c.execute(
                ...
                WHERE q.tg_user_id=? AND q.status='new'
                ORDER BY q.score DESC, v.published_at DESC
                LIMIT 1
                """,
                (tg_user_id,),
            ).fetchone()
