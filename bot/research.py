...
class ResearchService:
    ...
    def run_now(self, tg_user_id: int, period_days: int = 30) -> tuple[bool, int]:
        ...
        try:
            ...
            for row in self.hh.fetch_many(queries, period_days=period_days):
                ...
                title_l = vac.title.lower()
                if not any(x in title_l for x in ["аналитик", "analyst", "fraud", "risk", "bi "]):
                    continue
                ...
                if scored.score < self.settings.min_score_to_queue:
                    continue
                if self.db.add_to_queue(tg_user_id, vacancy_id, scored.score):
                    inserted += 1
