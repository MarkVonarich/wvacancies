...
class Scorer:
    ...
    def score(self, vacancy: Vacancy, weights: dict[str, float]) -> ScoreResult:
        ...
        for token in self.config.get("title_blocklist", []):
            if self._contains(title, token):
                return ScoreResult(0, [], [f"title-block: {token}"])
        ...
        analyst_like = any(k in title for k in ["analyst", "аналитик", "risk", "fraud", "bi ", "data analyst", "product analyst", "marketing analyst"])
        if role_hits == 0 and not analyst_like:
            return ScoreResult(0, plus[:3], ["no_target_role"])
        if analyst_like and role_hits == 0:
            plus.append("analyst_like")
            raw += 6
        ...
        if role_hits == 0:
            raw -= 20
            minus.append("no_exact_role")

        if neg_hits >= 2 and role_hits == 0:
            raw -= 20

        if not plus and role_hits == 0:
            return ScoreResult(0, [], ["no_positive_signals"])
